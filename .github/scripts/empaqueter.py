...
# -*- coding: utf-8 -*-
"""
Fabrique les fichiers d'une version : un ZIP par outil, dossier de l'outil à la racine, licence
jointe — le format que Claude demande à l'import —, et les notes tirées du journal des versions.
...
def main():
    ...
    liens = "\n".join(f"- [{o}.zip]({DEPOT}/releases/latest/download/{o}.zip)" for o in OUTILS)
    (sortie / "notes.md").write_text(
        "## Ce qui change\n\n" + (entree.group(0).strip() if entree else "Voir le journal des versions.") +
        "\n\n## Installer ou mettre à jour\n\n" +
        "Par le catalogue de plugins : vérifier les mises à jour. Par fichiers : réimporter ces ZIP, un par outil.\n\n" + liens + "\n",
        encoding="utf-8")
    print("OK        notes.md")
